...
-- Crear sinónimos privados para las tablas TRABAJADOR, BONO_ANTIGUEDAD y TICKETS_CONCIERTO:

CREATE OR REPLACE SYNONYM trab FOR TRABAJADOR;
CREATE OR REPLACE SYNONYM bono_antig FOR BONO_ANTIGUEDAD;
CREATE OR REPLACE SYNONYM tickets FOR TICKETS_CONCIERTO;


-- Realizar la consulta que cumple con los requisitos:


CREATE OR REPLACE VIEW V_SIMULACION_BONIFICACIONES AS
SELECT
  ...
  NVL(tt.MONTO_TOTAL_TICKETS, 0) AS MONTO_TOTAL_TICKETS,
  NVL(tt.BONIF_TOTAL_TICKETS, 0) AS BONIF_TOTAL_TICKETS,
  t.sueldo_base + NVL(tt.BONIF_TOTAL_TICKETS, 0) AS SIMULACION_SUELDO_TICKETS,
  t.sueldo_base * (1 + NVL(ba.porcentaje, 0)) AS SIMULACION_SUELDO_ANTIGUEDAD
FROM trab t
JOIN isapre i ON t.cod_isapre = i.cod_isapre
LEFT JOIN (
  SELECT
    numrut_t,
    SUM(monto_ticket) AS MONTO_TOTAL_TICKETS,
    SUM(
      CASE
        WHEN monto_ticket <= 50000 THEN 0
        WHEN monto_ticket > 50000 AND monto_ticket <= 100000 THEN monto_ticket * 0.05
        WHEN monto_ticket > 100000 THEN monto_ticket * 0.07
        ELSE 0
      END
    ) AS BONIF_TOTAL_TICKETS
  FROM tickets
  GROUP BY numrut_t
) tt ON t.numrut = tt.numrut_t
LEFT JOIN (
  SELECT
    t.numrut,
    ba.porcentaje
  FROM trab t
  LEFT JOIN bono_antig ba ON TRUNC(MONTHS_BETWEEN(SYSDATE, t.fecing)/12) BETWEEN ba.limite_inferior AND ba.limite_superior
) ba ON t.numrut = ba.numrut
...
-- Paso 1: Creando los sinonimos

CREATE OR REPLACE SYNONYM trab FOR TRABAJADOR;
CREATE OR REPLACE SYNONYM bono_esc FOR BONO_ESCOLAR;
CREATE OR REPLACE SYNONYM tipo_trab FOR TIPO_TRABAJADOR;
CREATE OR REPLACE SYNONYM asig_fam FOR ASIGNACION_FAMILIAR;


-- Paso 2: Creando la vista V_AUMENTOS_ESTUDIOS
...
SELECT
  ...
  b.porc_bono AS PORCENTAJE_ESTUDIOS,
  t.sueldo_base AS SUELDO_ACTUAL,
  t.sueldo_base * (b.porc_bono / 100) AS AUMENTO_CALCULADO,
  t.sueldo_base + (t.sueldo_base * (b.porc_bono / 100)) AS SIMULACION_SUELDO
FROM trab t
JOIN bono_esc b ON t.id_escolaridad_t = b.id_escolar
JOIN tipo_trab tt ON t.id_categoria_t = tt.id_categoria
LEFT JOIN (
  SELECT numrut_t, COUNT(*) AS num_cargas
  FROM asig_fam
  GROUP BY numrut_t
) af ON af.numrut_t = t.numrut
WHERE tt.desc_categoria = 'CAJERO' OR NVL(af.num_cargas, 0) IN (1, 2);



--Paso 3: Creacion de indices para optimizar consultas:
...
-- Creando Indices: Para optimizar estas consultas y evitar el Full Table Scan, crearemos dos índices:

CREATE INDEX idx_apmaterno ON TRABAJADOR(apmaterno);

CREATE INDEX idx_upper_apmaterno ON TRABAJADOR(UPPER(apmaterno));



-- Paso 4:  Ajustar la consulta para unir con la tabla ISAPRE

SELECT t.*, i.nombre_isapre
FROM TRABAJADOR t
JOIN ISAPRE i ON t.cod_isapre = i.cod_isapre
